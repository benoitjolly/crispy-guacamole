...
class TableComponent extends Component {
    ...
    dynamiqueTable = () => {
        ...
        if( this.props.dataAcq.length === 0 && this.props.dataMo.length === 0) return(<div>no data</div>);
          ...
          for (let b in data[a][arrayOfKey[0]]) {
              ...
              for (let c in data[a][arrayOfKey[0]][b][arrayOfKey[1]]) {
                  ...
                  let totalTotalSubRevenue3 = 0;
                    ...
                    totalSubrevenu += objMo.revenue;
                    totalSubRevenu2 += objMo.revenue;
                    totalTotalSubRevenue3 += objMo.revenue;
                    ...
                    table.unshift(data[a].open && data[a][arrayOfKey[0]][b].open && <tr className="over" key={`${a}${b}${c}`} onClick={() => this.props.onClick([a,b,c])}><td className="sub3"> {`${data[a][arrayOfKey[0]][b][arrayOfKey[1]][c].name}`}</td><td className="total">{-totalTotalSub3.toFixed(2)}</td><td>{totalTotalSubRevenue3.toFixed(2)}</td><td>{(totalTotalSubRevenue3-totalTotalSub3.toFixed(2)).toFixed(2)}</td></tr>);  
                }
                table.unshift(data[a].open && <tr className="over" key={`${a}${b}`} onClick={() => this.props.onClick([a,b])} ><td className="sub2"> {`${data[a][arrayOfKey[0]][b].name}`}</td><td className="total">{-totalSub2.toFixed(2)}</td><td>{totalSubRevenu2.toFixed(2)}</td><td>{(totalSubRevenu2-totalSub2).toFixed(2)}</td></tr>);
             }
             table.unshift(<tr className="over" key={`${a}`} onClick={() => this.props.onClick([a])}><td className="sub"> {`${data[a].name}`}</td><td className="total">{-totalSub1.toFixed(2)}</td><td>{totalSubrevenu.toFixed(2)}</td><td>{(totalSubrevenu-totalSub1).toFixed(2)}</td></tr>);
        }
